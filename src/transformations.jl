...
@views function within(
    ...
    while er > 1e-8
        for dimension in D
            current = copy(output)
            for group in dimension
                output[group] .-= mean(current[group], FrequencyWeights(wts[group]))
            end
        end
        er = √sum((x - y)^2 for (x, y) in zip(output, current))
    end
    output .+= μ
end
"""
...
"""
@views function partialwithin(
    obj::AbstractVecOrMat{<:Real},
    ...
    output = copy(obj)
    if !isempty(wts)
        for (panel, θ) in zip(panel, θ)
            output[panel, :] .-=
                θ * mean(obj[panel, :], FrequencyWeights(wts[panel]), dims = 1)
        end
    else
        for (panel, θ) in zip(panel, θ)
...
transform(
    estimator::ContinuousResponse,
    obj::AbstractVecOrMat{<:Number},
    wts::FrequencyWeights,
) = isempty(obj) ? obj : within(obj, estimator.groups, wts)
transform(
    ...
    mapreduce(
    group -> mean(obj[group], FrequencyWeights(wts[group])),
    vcat,
    estimator.groups,
)
transform(
    ...
    mapreduce(
    group -> mean(obj[group, :], FrequencyWeights(wts[group]), dims = 1),
    vcat,
    estimator.groups,
)
transform(estimator::RandomEffectsEstimator, obj::AbstractVecOrMat, wts::FrequencyWeights) =
    isempty(obj) ? obj : partialwithin(obj, estimator.pid[2], estimator.θ, wts)
