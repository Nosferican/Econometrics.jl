...
function decompose(
    ...
    elseif isa(estimator, Type{<:RandomEffectsEstimator}) &&
           (isnothing(panel) || isnothing(time))
        throw(ArgumentError("The random effects estimator requires the panel and temporal identifiers."))
    elseif (
        isa(estimator, Type{<:BetweenEstimator}) ||
        isa(estimator, Type{<:RandomEffectsEstimator})
    ) && !isempty(absorbed)
    ...
    categorical_variables =
        Tables.schema(data) |> (
            x -> [
                name
                for
                (name, type) ∈ zip(x.names, x.types) if
    ...
    elseif length(absorbed) == 1
        ...
        if isa(absorbed, AbstractVector)
            absorbed = unique(flatten(termvars(t) for t in absorbed))
        else
            absorbed = termvars(absorbed)
        end
        absorbed = [
            [
                findall(isequal(l), getproperty(data, dim))
                for l in levels(getproperty(data, dim))
            ] for dim in absorbed
        ]
    else
    ...
    if isa(estimator, Type{<:RandomEffectsEstimator})
        panel = (
            panel,
            [
                findall(isequal(l), getproperty(data, panel))
                for l in levels(getproperty(data, panel))
            ],
        )
        time = (
            time,
            [
                findall(isequal(l), getproperty(data, time))
                for l in levels(getproperty(data, time))
            ],
        )
        estimator = RandomEffectsEstimator(panel, time, X, y, z, Z, FrequencyWeights(wts))
    elseif isa(estimator, Type{<:BetweenEstimator})
        estimator = BetweenEstimator(
            panel,
            [
                findall(isequal(l), getproperty(data, panel))
                for l in levels(getproperty(data, panel))
    ...
    elseif isa(estimator, Type{<:EconometricModel}) ||
           isa(estimator, Type{<:ContinuousResponse})
...
clean_fm(obj::EconometricsModel) =
    obj.f |> (f -> string(clean_lhs(f.lhs), " ~ ", clean_rhs(f.rhs))) |> (
        f -> isa(obj, EconometricModel{<:Union{RandomEffectsEstimator,OrdinalResponse}}) ?
